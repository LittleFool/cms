...
class iha {
     ...
     * 
     * Usage:
     *      $iha->compare("secret password", "AAAJxA;2Rd/Ts;3qwiKTWppNtb2M4nhDrmyA==");
     * 
     * @param String $pwd
     * @param String $hash
     * @return boolean 
     ...
     * @return String 
     */
    private function generateHash($pwd, $salt, $sha1, $rounds) {
        if (empty($salt)) {
     ...
     * @return String 
     */
    private function keyStretching($key, $sha1, $rounds) {
        if ($sha1 == true AND function_exists("sha1")) {
            for ($i = 0; $i < $rounds; ++$i) {
                $key = sha1($key);
            }
